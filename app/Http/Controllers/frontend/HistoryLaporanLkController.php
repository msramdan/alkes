...
class HistoryLaporanLkController extends Controller
{
    ...
    private function laporanData($faskes, $status)
    {
        ...
        if (isset($status) && !empty($status)) {
            if ($status != 'all') {
                $laporandata = $laporandata->where('laporans.status_laporan', $status);
            }
        }
        return $laporandata->orderBy('laporans.no_laporan', 'DESC')->paginate(5);
    }

    public function filter()
    {
        $selected_status = request()->query('selected_status') ?? 'all';
        $selected_faskes = request()->query('selected_faskes') ?? 'all';
        $laporandata = $this->laporanData(intval($selected_faskes), $selected_status);
        $faskes = Faske::orderBy('nama_faskes', 'ASC')->get();
        return view('frontend.history-laporan.index', [
            'laporan' =>  $laporandata,
            'faskes' =>  $faskes,
            'selected_status' => $selected_status,
            'selected_faskes' => intval($selected_faskes)
        ]);
    }
