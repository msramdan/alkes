...
$flowmeter = DB::table('laporan_kinerja')->where('type_laporan_kinerja', 'flowmeter_oksigen_concentrator')->where('no_laporan', $laporan->no_laporan)->first();
$data_sertifikat = json_decode($flowmeter->data_sertifikat);
$data_laporan = json_decode($flowmeter->data_laporan);

$flowMeters = [2, 4, 6, 8, 10];
$flowMeterData = [];
$flowMeterScore = 0;
...
$flowMeterFinalScore = ($flowMeterScore / count($flowMeters)) * 100;
$flowMeterRequirement = $flowMeterFinalScore >= 70 ? 'Lulus' : 'Tidak';
?>

{{-- konsentrasi oksigen --}}
<?php
$oksigen = DB::table('laporan_kinerja')->where('type_laporan_kinerja', 'konsentrasi_oksigen_concentrator')->where('no_laporan', $laporan->no_laporan)->first();
$data_sertifikat_oksigen = json_decode($oksigen->data_sertifikat);
$data_laporan_oksigen = json_decode($oksigen->data_laporan);
...
foreach ($oxygenSettings as $setting) {
    $prefix = "konsentrasi_oksigen_{$setting}";
    $measurements = [];

    // Ambil 6 pengukuran
    for ($i = 1; $i <= 6; $i++) {
        $field = "{$prefix}_{$i}";
        $measurements[$i] = $data_laporan_oksigen->$field;
    }
    ...
    $stdev = standard_deviation($measurements);
    $u95 = hitung_uncertainty($resolusi->value, $stdev, 0, 0, 6);
    $correction = $mean_terkoreksi - $setting;
    $cu95 = abs($correction) + $u95;
    $tolerance = 3;
    $result = abs($u95) <= $tolerance ? 'Lulus' : 'Tidak';
...
$oksigenFinalScore = ($oksigenScore / count($oxygenSettings)) * 100;
$oksigenRequirement = $oksigenFinalScore >= 70 ? 'Lulus' : 'Tidak';
$score_kinerja = (($flowMeterFinalScore + $oksigenFinalScore) / 2) * 0.5;
...
<table class="table table-bordered table-sm"
    ...
    @foreach ($flowMeterData as $data)
        <tr>
            <td style="text-align: center;vertical-align: middle;">{{ $data['setting'] }}</td>
            @for ($i = 1; $i <= 6; $i++)
...
<p style="font-size: 11px;margin-left:18px"><b>Konsentrasi Oksigen</b></p>
<table class="table table-bordered table-sm"
    style="margin-left: 18px;font-size:9px;width:100%;margin-top:-10px; padding-right:18px">
    <tr>
        <th rowspan="2" style="text-align: center;vertical-align: middle;">Setting Standar</th>
    ...
    @foreach ($oksigenData as $data)
        <tr>
            ...
            <td style="text-align: center;vertical-align: middle;">{{ $data['result'] }}</td>
            @if ($data['setting'] == 5)
                <td style="text-align: center;vertical-align: middle;">{{ $oksigenFinalScore }}</td>
                <td style="text-align: center;vertical-align: middle;">{{ $oksigenRequirement }}</td>
            @endif
        </tr>
    @endforeach
</table>
