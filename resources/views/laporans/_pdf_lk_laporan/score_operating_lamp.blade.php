...
$intensitas_cahaya = DB::table('laporan_kinerja')->where('type_laporan_kinerja', 'intensitas_cahaya_lengan_1')->where('no_laporan', $laporan->no_laporan)->first();
$data_sertifikat = json_decode($intensitas_cahaya->data_sertifikat);
$data_laporan = json_decode($intensitas_cahaya->data_laporan);
$arrString = ['cahaya_lengan_1'];
$myArrayString = [];
$initScoreString = 0;
...
$scoreKinerja = $scoreString;

// Intensitas Cahaya Lengan 2
$intensitas_cahaya2 = DB::table('laporan_kinerja')->where('type_laporan_kinerja', 'intensitas_cahaya_lengan_2')->where('no_laporan', $laporan->no_laporan)->first();
$data_sertifikat2 = json_decode($intensitas_cahaya2->data_sertifikat);
$data_laporan2 = json_decode($intensitas_cahaya2->data_laporan);
$arrString2 = ['cahaya_lengan_2'];
$myArrayString2 = [];
$initScoreString2 = 0;

foreach ($arrString2 as $value) {
    // 1000
    $a2 = $value . '_1';
    $$a2 = $data_laporan2->$a2;
    ...
    // mean
    $mean2 = 'mean_' . $value;
    $$mean2 = ($$a2 + $$b2 + $$c2 + $$d2 + $$e2 + $$f2) / 6;

    // mean terkoreksi
    $mean_terkoreksi2 = 'mean_terkoreksi_' . $value;
    $$mean_terkoreksi2 = $data_sertifikat2->intercept + $data_sertifikat2->slope * $$mean2;
    // stdev
    $arrData2 = [];
    array_push($arrData2, $$a2, $$b2, $$c2, $$d2, $$e2, $$f2);

    $stdev2 = standard_deviation($arrData2);
    $var_stdev2 = 'stdev' . $value;
    $$var_stdev2 = $stdev2;

    // U95
    $u952 = 'u95' . $value . '_naik';
    // U95
    $$u952 = hitung_uncertainty($resolusi->value, $$var_stdev2, 2.6, 0.01, 6);
    // cu95
    $cu952 = 'abs95' . $value;
    $$cu952 = $$mean2 + $$u952;

    // toleransi
    $toleransi2 = 'toleransi' . $value;
    $$toleransi2 = 40;

    // hasil
    $hasil2 = 'hasil' . $value;
    $$hasil2 = $$cu952 >= $$toleransi2 ? 'Lulus' : 'Tidak';
    if ($$hasil2 == 'Lulus') {
        $initScoreString2 = $initScoreString2 + 1;
    }
    ...
    $data = [
        ...
        'percobaan_6' => $$f2,
        'mean' => $$mean2,
        'mean_terkoreksi' => $$mean_terkoreksi2,
        'stdev' => $$var_stdev2,
        'koreksi' => '',
        'u95' => $$u952,
        'cu95' => $$cu952,
    ...
    $myArrayString2[$value] = $data;
    $arrDataString2 = [];
}

$scoreString2 = (($initScoreString2 / 1) * 100) / 2;
$persyaratanString2 = $scoreString2 >= 50 ? 'Lulus' : 'Tidak';
$scoreKinerja2 = $scoreString2;
...
<p style="font-size: 11px;margin-left:18px"><b> Intensitas Cahaya Lengan 1</b></p>
<table class="table table-bordered table-sm"
...
<p style="font-size: 11px;margin-left:18px"><b> Intensitas Cahaya Lengan 2</b></p>
<table class="table table-bordered table-sm"
    style="margin-left: 18px;font-size:9px;width:100%;margin-top:-10px; padding-right:18px">
    <tr>
        <th rowspan="2"style="text-align: center;vertical-align: middle;">Setting Alat</th>
    ...
    @foreach ($myArrayString2 as $key => $value)
        <tr>
            <td style="text-align: center;vertical-align: middle;">MAX</td>
            <td style="text-align: center;vertical-align: middle;">{{ $value['percobaan_1'] }}</td>
            <td style="text-align: center;vertical-align: middle;">{{ $value['percobaan_2'] }}</td>
    ...
    <tbody>
        <tr>
            <td style="height:70px" style="text-align: justify">Berdasarkan Metode Kerja
                ...
                <?php
                $totalAll = $score_fisik + $point + (($scoreKinerja + $scoreKinerja2 )/2);
                ?>
