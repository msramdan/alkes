...
<table class="table table-bordered" style="border-color: black">
    ...
    <tbody>
        @foreach ([1, 10, 15, 12, 15] as $index => $settingAlat)
            <tr>
                <td>{{ $settingAlat }}</td>
                @for ($j = 1; $j <= 6; $j++)
                    <td>
                        <input type="number" step="0.000000001" class="form-control"
                               name="percobaan{{ $index + 1 }}_{{ $j }}" required style="width: 100px">
                    </td>
                @endfor
                @if ($index == 0)
                    <td rowspan="5">± 10%</td>
        ...
        @foreach ([21, 50, 100] as $index => $settingAlat)
            <tr>
                ...
                @for ($j = 1; $j <= 6; $j++)
                    <td>
                        <input type="number" step="0.000000001" class="form-control"
                               name="percobaan_konsentrasi_oksigen{{ $index + 1 }}_{{ $j }}" required style="width: 100px">
                    </td>
                @endfor
                @if ($index == 0)
